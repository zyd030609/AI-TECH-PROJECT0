...
class Board(object):
    ...
    def __init__(self, size=(9, 9)):
        self.state = torch.zeros(size, dtype=torch.float32, device='cuda')  # 初始化棋盘状态到 GPU

    def update_state(self, action):
        # 更新状态，假设 action 是一个坐标
        self.state[action] = 1  # 假设 1 表示棋子

    def is_terminal(self):
        """
        检查棋局是否终局。
        :return: bool, 如果棋局结束返回 True，否则返回 False。
        """
