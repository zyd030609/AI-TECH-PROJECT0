...
# '''左亦东（11.24）：待解决的问题：解决方案：
# 1.界面不能判断当前走子方：添加侧边栏，展示红黑方信息。√
# 2.人类玩家（Human类）走子规则约束错误：修改对照表对象，改为与Board.availables进行比较。√
# 3.人机对战时，人类玩家可以操纵对方玩家的棋子：同上。√
# '''
...
class Human:
    ...
    def get_action(self, move):
        ...
        if  move_action2move_id.__contains__(move):
            ...
            #左亦东（11.24）：2、3
            if board.availables.__contains__(move):
                return move
            else:
                return -1
...
pygame.init()
pygame.mixer.init()
pygame.mixer.music.load('bgm/高山流水.mp3')
...
while True:
    ...
    panel_rect = pygame.Rect(screen.get_width() - right_panel_width, 0, right_panel_width, screen.get_height())
    # 绘制白色背景
    pygame.draw.rect(screen, (255, 255, 255), panel_rect)
    # 打印红黑方信息
    font = pygame.font.SysFont('Arial', 20)
    # 如果需要打印动态信息，可进一步调用玩家信息相关方法
    red_info = "red:AI" if isinstance(player1, MCTSPlayer) else "red:Human"
    black_info = "black:AI" if isinstance(player2, MCTSPlayer) else "black:Human"
    current_player_info = "current:red" if board.get_current_player_id()==1 else "current:black"
    # 打印动态信息
    text_red_info = font.render(red_info, True, (0, 0, 0))
    text_black_info = font.render(black_info, True, (0, 0, 0))
    text_current_player_info = font.render(current_player_info, True, (0, 0, 0))
    screen.blit(text_red_info, (screen.get_width() - right_panel_width + 10, 20))
    screen.blit(text_black_info, (screen.get_width() - right_panel_width + 10, 60))
    screen.blit(text_current_player_info, (screen.get_width() - right_panel_width + 10, 120))
