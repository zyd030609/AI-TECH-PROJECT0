...
class TrainPipeline:
    ...
    def policy_evaluate(self, n_games=10): #测试模型，没看懂。=>模拟对战胜率
        """
    ...
    def policy_updata(self):
        ...
        mini_batch = [zip_array.recovery_state_mcts_prob(data) for data in mini_batch] #=>[(state,mcts_prob,winner),]
        state_batch = [data[0] for data in mini_batch]
        state_batch = np.array(state_batch).astype('float32') #list of array[][][]->array[][][][]
        ...
        for i in range(self.epochs): #拟合某一批数据，一定步数，不是完全拟合
        ...
        try:
            for i in range(self.game_batch_num):
                ...
                if (i + 1) % self.check_freq == 0:
                    ...
                    self.policy_value_net.save_model('models/current_policy_batch{}.model'.format(i + 1)) #保存不同训练次数时的模型
