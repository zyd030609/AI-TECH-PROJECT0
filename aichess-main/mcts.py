...
class TreeNode(object):
    ...
    def __init__(self, parent, prior_p):
        ...
        """
        self._parent = parent
        self._children = {} # 从动作到TreeNode的映射.dict,action:TreeNode
...
class MCTS(object):
    ...
    def get_move_probs(self, state, temp=1e-3): #运行完2000次搜索后得到的统计概率，而非策略价值网络生成
        """
        ...
        act_visits= [(act, node._n_visits)
                     for act, node in self._root._children.items()] #只取根节点的
        acts, visits = zip(*act_visits) #*：解包运算符
        act_probs = softmax(1.0 / temp * np.log(np.array(visits) + 1e-10))
        return acts, act_probs

    def update_with_move(self, last_move): #落子，新的搜索树（新的根节点）
        """
...
# 基于MCTS的AI玩家
class MCTSPlayer(object): #ai玩家类，蒙特卡洛树是其成员变量
    ...
    # 得到行动
    def get_action(self, board, temp=1e-3, return_prob=0): #取得当前盘面的下一步动作，=>选择的下一步动作，所有可选动作的概率
        # 像alphaGo_Zero论文一样使用MCTS算法返回的pi向量
        move_probs = np.zeros(2086)

        acts, probs = self.mcts.get_move_probs(board, temp) #以该状态为根节点，先进行2000次的模拟，创建出搜索树，得到该状态的子节点
        ...
        if self._is_selfplay:
            ...
            # 重置根节点
            self.mcts.update_with_move(-1) #重置而非更新，因为对方是真人
        if return_prob:
