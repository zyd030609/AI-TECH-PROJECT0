...
class CollectPipeline:
    ...
    def collect_selfplay_data(self, n_games=1): #执行一轮完整的自我博弈（直到胜利）
        # 收集自我对弈的数据
        for i in range(n_games):
            ...
            self.episode_len = len(play_data) #走到赢的步数
            # 增加数据
            ...
            if CONFIG['use_redis']:
                while True:
                    try:
                        for d in play_data:
                            self.redis_cli.rpush('train_data_buffer', pickle.dumps(d)) #pickle：用于处理二进制文件
                        self.redis_cli.incr('iters') #iters+=1
                ...
                if os.path.exists(CONFIG['train_data_buffer_path']):
                    while True:
                        try:
                            with open(CONFIG['train_data_buffer_path'], 'rb') as data_dict:
                                ...
                                self.data_buffer = deque(maxlen=self.buffer_size)
                                self.data_buffer.extend(data_file['data_buffer']) #取出缓冲文件的数据到队列
                                self.iters = data_file['iters']
                                del data_file #然后删了原缓冲文件
                                self.iters += 1
                                self.data_buffer.extend(play_data) #加上新的数据
                            print('成功载入数据')
                            break
                        except:
                            time.sleep(30)
                    ...
                    self.iters += 1
            data_dict = {'data_buffer': self.data_buffer, 'iters': self.iters} #用了数据库，缓冲文件就是空
