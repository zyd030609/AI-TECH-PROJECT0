...
def rollout_policy_fn(board):
    """在 GPU 上生成随机概率"""
    availables = list(board.availables)  # 转换为列表
    action_probs = torch.rand(len(availables), device='cuda')  # 在 GPU 上生成随机数
    return zip(availables, action_probs.cpu().numpy())  # 转为 numpy 数组返回
